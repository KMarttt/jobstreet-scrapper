...
def setup_logging(is_scraping, site, portal, location, keyword):
    if is_scraping:
        log_file_name = f"terminal_output_{site}_{portal}_{location}_{keyword}_rescraped.txt"
    else:
        log_file_name = f"terminal_output_{site}_{portal}_{location}_{keyword}.txt"
...
async def extract_job_links(page, portal, site, location, keyword, max_pages):
...
async def web_scraper(is_rescraping, link_file_name, portal="my", site="jobstreet", location="", keyword="Data-Analyst", max_pages=2):
    # Phase 1: Initiate    
    ...
    async with async_playwright() as pw:
        ...
        if is_rescraping:
            ...
            job_links = await extract_job_links(page, portal, site, location, keyword, max_pages)
        ...
        if is_rescraping:
            csv_file = f"data/{site}_{portal}_{location}_{keyword}_rescraped.csv"
            csv_error_file = f"data/{site}_{portal}_{location}_{keyword}_rescraped_error.csv"
        else:
            csv_file = f"data/{site}_{portal}_{location}_{keyword}.csv"
            csv_error_file = f"data/{site}_{portal}_{location}_{keyword}_link_errors.csv"
...
if __name__ == "__main__":
    ...
    rescrape_input = input("Are you here to rescrape? (Y/N): ").lower().strip()
    ...
    # Terminal Logging
    log_file = setup_logging(is_rescraping, site, portal, location, keyword)

    # Proper Run
    asyncio.run(web_scraper(is_rescraping, link_file_name, portal, site, location, keyword, max_pages))
